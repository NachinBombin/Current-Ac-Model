...
do
    for i = 1, RICO_BUF_SIZE do
        bombin_gau_rico_store.buffer[i] = {
            pos      = Vector(0,0,0),
            old_pos  = Vector(0,0,0),
            vel      = Vector(0,0,0),
            old_vel  = Vector(0,0,0),
...
local function bombin_spawn_visual_rico(hitPos, hitNormal)
    local store    = bombin_gau_rico_store
...
net.Receive("bombin_gau_rico", function()
    local hitPos    = net.ReadVector()
    local hitNormal = net.ReadVector()
    bombin_spawn_visual_rico(hitPos, hitNormal)
end)
...
local function gau_check_passby(proj)
    ...
    if sign_old <= 0 then proj.gau_wizz = true return end
...
local function move_cl()
    ...
    -- ─ visual ricochets ─
    local visuals = bombin_gau_rico_store.active_visuals
    local vc      = #visuals
    local vi      = 1
    local now     = CurTime()
    while vi <= vc do
...
local function render_projectiles()
    ...
    if count > 0 then
        for i = 1, count do
            ...
            local scale = m_clamp(dist / 1200, 1.5, 6)
    ...
    -- ─ visual ricochets ─
    -- Same brightness/scale logic as AC47. Colors inherit the GAU yellow-white family
    -- so ricochets read as hotter sparks of the same round, not foreign tracers.
    local visuals = bombin_gau_rico_store.active_visuals
    local vc      = #visuals
    if vc > 0 then
        local now = CurTime()
        for i = 1, vc do
            ...
            render.SetMaterial(mat_beam)
            if render_pos:DistToSqr(tail_end) > 4 then
                -- Core: GAU yellow-white family, brighter/wider than base tracer
