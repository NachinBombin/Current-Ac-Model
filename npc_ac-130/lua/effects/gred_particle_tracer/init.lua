...
function EFFECT:Init(data)
    ...
    local particle = CreateParticleSystem(
        World,
        "gred_tracers_" .. COL_TABLE[data:GetMaterialIndex()] .. "_" .. CAL_TABLE[cal],
        PATTACH_WORLDORIGIN,
        0,
        pos
    )
    ...
    -- store a reference on the effect so it is not GC'd
    self.Particle = particle

    particle:SetControlPoint(1, endpos)

    local life = (pos:DistToSqr(endpos) * 6.8 / SPEED_TABLE[cal])

    timer.Simple(life, function()
        -- effect or particle may be gone by the time the timer fires
