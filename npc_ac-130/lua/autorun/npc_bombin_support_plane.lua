...
if SERVER then
	...
	local function SpawnSupportPlaneAtPos(centerPos, callDir)
		...
		-- Use NW vars so Initialize() can reliably read them before Spawn()
		plane:SetNWVector("CenterPos",    centerPos)
		plane:SetNWVector("CallDir",      callDir)
		plane:SetNWFloat ("Lifetime",     cv_life:GetFloat())
		plane:SetNWFloat ("Speed",        cv_speed:GetFloat())
		plane:SetNWFloat ("OrbitRadius",  cv_radius:GetFloat())
		plane:SetNWFloat ("SkyHeightAdd", cv_height:GetFloat())
		plane:Spawn()
		plane:Activate()
