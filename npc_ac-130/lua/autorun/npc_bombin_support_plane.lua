if SERVER then
	...
	local function CheckSkyAbove(pos)
		local trace = util.TraceLine({ start = pos + Vector(0,0,50), endpos = pos + Vector(0,0,1050) })
		if trace.Hit and not trace.HitSky then
			trace = util.TraceLine({ start = trace.HitPos + Vector(0,0,50), endpos = trace.HitPos + Vector(0,0,1000) })
		end
	...
	local function ThrowSupportFlare(npc, targetPos)
		...
		timer.Simple(0, function()
			...
			if not IsValid(phys) then BSP_Debug("Flare physics invalid after spawn") return end
	...
	local function FireBombinSupportPlane(npc, target)
		...
		if not IsValid(target) or not target:IsPlayer() or not target:Alive() then
			BSP_Debug("Call rejected: target invalid") return false
		end

		local targetPos = target:GetPos() + Vector(0, 0, 36)
		if not CheckSkyAbove(targetPos) then BSP_Debug("Call rejected: no open sky above target") return false end
		...
		if not IsValid(flare) then BSP_Debug("Call rejected: flare could not be created") return false end

		local fallbackPos = Vector(targetPos.x, targetPos.y, targetPos.z)
		local storedDir   = Vector(callDir.x,   callDir.y,   callDir.z)

		BSP_Debug("Flare deployed, waiting " .. tostring(cv_delay:GetFloat()) .. "s before plane spawn")
